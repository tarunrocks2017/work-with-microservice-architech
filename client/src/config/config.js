...
const IP = '18.222.132.13' ;
if(process.env.NODE_ENV === 'production') {
     url.GET_MOVIE_URL = `http://${IP}:8080/movies`;
     url.GET_ACTOR_URL = `http://${IP}:8080/actors`;
     url.ADD_ACTOR_URL = `http://${IP}:8080/addActor`;
     url.ADD_MOVIE_URL = `http://${IP}:8080/addMovie`;
     url.DELETE_ACTOR_URL = `http://${IP}:8080/deleteActor/`;
     url.DELETE_MOVIE_URL = `http://${IP}:8080/deleteMovie/`;
     url.EDIT_ACTOR_URL = `http://${IP}:8080/updateActor`;
     url.EDIT_MOVIE_URL =`http://${IP}:8080/updateMovie`;
}
